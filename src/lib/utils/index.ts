...
/**
 * 2024-07-26
 * @desc  随机命中给定数组的若干条作为结果返回
 * @param arr
 * @param items
 * @returns array
 */
export function getRandomItems(arr = [], items = 1) {
  if (!items || items > arr.length) {
    items = 1
  }
...
export const getShortenMidDots = (str: string, length = 8) => {
  if (!str) return
  return str?.length > 25 ? `${str?.slice(0, length)}...${str?.slice(-length)}` : str
}
/**
...
export const getNFTOrScanUrl = ({
  ...
  let _item = nftAvailableChains.find(row => row.chain == chain?.toLowerCase()),
    ...
    _sol = chainType == 'solana' || chain?.toLowerCase()?.includes('sol'), // 兼容不同地方的solana数据
    _icp = chainType == 'icp'
...
export const getItemBgColorData = type => {
...
export function getSocialLinks(data: Links[] = []): { type: string; link: string; handle: string }[] {
  if (!data.length) return []

  const flattenedLinks = data.flatMap(links =>
    Object.entries(links).map(([type, { link, handle }]: [string, SocialLink]) => ({
      type,
      link,
      handle,
      bg: getItemBgColorData(type),
    }))
  )

  const uniqueLinksMap = new Map()
  flattenedLinks.forEach(link => {
    if (!uniqueLinksMap.has(link.type)) {
      uniqueLinksMap.set(link.type, link)
    }
  })
...
export function getSocialList(list: any[]) {
  let platforms = ['ens', 'farcaster', 'lens', 'dotbit', 'unstoppabledomains', 'sns']
  if (list && list.length) {
    return list.reduce(
      (acc, row) => {
      ...
      {
        data: list.filter(row => platforms.includes(row.platform)),
        links: getSocialLinks(list.map(item => item.links)),
      }
    )
  }
  return null
