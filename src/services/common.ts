...
/**
 * Web3 Domain Name Service Resolver
 * https://api.web3.bio
 * @param params address
 * @param type profile | ns
 * @param platform '' | ens | farcaster | lens | sns | botbit | unstoppabledomains
 * @returns
 */

export function getBioNamesSvc(params, type = 'profile', platform = '') {
  if (!params?.address) return
  const { address } = params
  return fetcher({
    baseURL: `https://api.web3.bio/${type}/${platform ? platform + '/' : ''}${address}`,
  })
}

export function getFarcasterAccountSvc(params) {
  if (!params?.address) return
  const { address } = params
  return fetcher({
    baseURL: `https://api.web3.bio/profile/farcaster/${address}`,
  })
}

export function getLensAccountSvc(params) {
  if (!params?.address) return
  const { address } = params
  return fetcher({
    baseURL: `https://api.web3.bio/profile/lens/${address}`,
  })
}
