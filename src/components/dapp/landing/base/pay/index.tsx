...
import { useChainConnect, useEVMWalletConnect, useGlobalWalletConnect, useSolAccount, useUserLocal } from '@/lib/hooks'
import { payChains, _supportChains } from '@/lib/chains'
import { getActiveChain, getSolanaRPCUrl } from '@/lib/web3'
import { env } from '@/lib/types/env'
import * as pay from '@/lib/chains/tokens'
import { wagmiCoreConfig } from '@/lib/chains/wagmi'
...
const PaymentCard = ({ ...props }) => {
  const { addressList } = props?.user || useUserLocal()
              ...
              {tokenListLoading ? (
                ...
                tokens?.list.map((row, index) => (
                  ...
                  >
                    ...
                    <ul className="flex-1 flex items-center px-2 gap-2">
                      <li>
                        <h3 className="truncate font-medium text-justify">{row?.symbol}</h3>
                ...
                <li className="text-center mt-36">
                  ...
                  <p>
                    ...
                    <NextLink href="/" target="_blank" rel="noopener noreferrer nofollow" className="px-1.5">
                      feedback
                    </NextLink>
          ...
          {payChains.map((item, index) => {
            let avatarBox = ({ classes = null } = {}) => (
              <Avatar
                ...
                className={classNames(chainAvatarClass, classes, item?.disabled ? 'blur-sm' : 'cursor-pointer')}
              />
            )
            return (
